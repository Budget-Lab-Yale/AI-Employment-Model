...
def build_other_output(ID: str, start: str, end: str, out_path: str, metric = "mean_rating_human_beta", freq = 3):
    ...
    # Same kind of filtering business we were doing earlier

    e_block = block[block["group"]=="EMPSTAT"]
    e_block = pandas.melt(e_block, id_vars="MONTH", value_vars=["percent_exposed", "percent_highly_exposed"], ignore_index=False)
    e_block = e_block.rename(columns={'variable': "Metric"})
    e_block.to_csv(os.path.join(this_dir, "exposure.csv"), index = False)
    plt.figure()
    ...
    u_block = block[block["group"] == "DURUNEMP_cat"]
    u_block = pandas.melt(u_block, id_vars=["MONTH", "group_val"], value_vars=["tasks_exposed_pct"], ignore_index=False)
    u_block = u_block.rename(columns={'group_val': "Duration of Unemployment"})
    u_block.to_csv(os.path.join(this_dir, "unemployment_exposure.csv"), index = False)
    plt.figure()
    ...
    s_block = block[block["group"] == "SEX"]
    s_block = pandas.melt(s_block, id_vars=["MONTH", "group_val"], value_vars=["percent_exposed", "percent_highly_exposed"], ignore_index=False)
    s_block = s_block.rename(columns={'group_val': "Sex", "variable": "Metric"})
    s_block.to_csv(os.path.join(this_dir, "sex_exposure.csv"), index = False)
    plt.figure()
    sns.lineplot(
        data = s_block,
        x = "MONTH",
    ...
    e_cat_block = e_cat_block.rename(
        columns = {
            "group": "Exposure Type",
            "group_val": "Exposed"
        }
    ) 
    e_cat_block.to_csv(os.path.join(this_dir, "age_exposure.csv"), index = False)
    plt.figure()
    sns.lineplot(
        data = e_cat_block,
        x = "MONTH",
